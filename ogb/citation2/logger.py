...
class Logger(object):
    ...
    def write_statistics(self):
        ...
        with open('citation2_results.txt', 'w') as f:
            f.write(f'All runs:\n')
            r = best_result[:, 0]
            f.write(f'Highest Train: {r.mean():.4f} ± {r.std():.4f}\n')
            r = best_result[:, 1]
            f.write(f'Highest Valid: {r.mean():.4f} ± {r.std():.4f}\n')
            r = best_result[:, 2]
            f.write(f'  Final Train: {r.mean():.4f} ± {r.std():.4f}\n')
            r = best_result[:, 3]
            f.write(f'   Final Test: {r.mean():.4f} ± {r.std():.4f}\n')
