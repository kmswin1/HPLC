...
def train(model, predictor, data, split_edge, optimizer, batch_size):
    ...
    for perm in DataLoader(range(source_edge.size(0)), batch_size,
        ...
        pos_out = predictor(torch.cat([h[src], data.se[src]], dim=-1), torch.cat([h[dst], data.se[dst]], dim=-1))
        pos_loss = -torch.log(pos_out + 1e-15).mean()

        # Just do some trivial random sampling.
        dst_neg = torch.randint(0, data.num_nodes, src.size(),
                                dtype=torch.long, device=h.device)
        neg_out = predictor(torch.cat([h[src], data.se[src]], dim=-1), torch.cat([h[dst_neg], data.se[dst_neg]], dim=-1))
...
def test(model, predictor, data, split_edge, evaluator, batch_size):
    ...
    def test_split(split):
        ...
        for perm in DataLoader(range(source.size(0)), batch_size):
            ...
            pos_preds += [predictor(torch.cat([h[src], data.se[src]], dim=-1), torch.cat([h[dst], data.se[dst]], dim=-1)).squeeze().cpu()]
        ...
        for perm in DataLoader(range(source.size(0)), batch_size):
            ...
            neg_preds += [predictor(torch.cat([h[src], data.se[src]], dim=-1), torch.cat([h[dst_neg], data.se[dst_neg]], dim=-1)).squeeze().cpu()]
...
def main():
    ...
    data['se'] = torch.tensor(se, dtype=torch.float32)
