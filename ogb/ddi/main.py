...
def train(model, predictor, x, adj_t, split_edge, optimizer, batch_size, data, pos_emb, lap_pe):
    row, col, _ = adj_t.coo()
    ...
    for perm in DataLoader(range(pos_train_edge.size(0)), batch_size,
                           shuffle=True):
        optimizer.zero_grad()
        h = model(x, adj_t, data.com_x, pos_emb, lap_pe)
...
@torch.no_grad()
def test(model, predictor, x, adj_t, split_edge, evaluator, batch_size, data, pos_emb, lap_pe):
    model.eval()
    predictor.eval()
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            ...
            loss = train(model, predictor, emb.weight, adj_t, split_edge,
                         optimizer, args.batch_size, data, data.pos_enc, data.lap_pe)

            if epoch % args.eval_steps == 0:
                results = test(model, predictor, emb.weight, adj_t, split_edge,
                               evaluator, args.batch_size, data, data.pos_enc, data.lap_pe)
            ...
            h = model(emb.weight, adj_t, data.com_x, data.pos_enc, data.lap_pe)
