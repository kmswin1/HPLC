...
from torch_geometric.nn import GCNConv, SAGEConv, JumpingKnowledge, GATConv, GINConv
...
class HPLC(nn.Module):
    def __init__(self, dim_feat, dim_h, dim_z, dropout, num_local, dim_pos, gnn_type='GCN', jk_mode='mean', dec='mlp'):
        ...
        if jk_mode == 'cat':
            ...
            dim_in = dim_z
        self.decoder = Decoder(dec, dim_pos, dim_in)
        self.init_params()
        self.phi = GINConv(1, num_local)
        self.rho = nn.Linear(num_local, num_local)

    def forward(self, adj, features, edges, com_xs, pos_emb, lap_pe, se):
        lap_pe = lap_pe.unsqueeze(-1)
        lap_pe = self.phi()
        z = self.encoder(adj, features, com_xs, pos_emb, lap_pe)
        z_i = torch.cat([z[edges.T[0]], se[edges.T[0]]], dim=-1)
        z_j = torch.cat([z[edges.T[1]], se[edges.T[1]]], dim=-1)
...
class Decoder(nn.Module):
    def __init__(self, dec, dim_pos, dim_z=256, dim_h=256):
        super(Decoder, self).__init__()
        self.dec = dec
        dim_in=dim_h + dim_pos
