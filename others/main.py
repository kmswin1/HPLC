...
def jaccard(list1, list2):
    ...
    return np.array([float(intersection) / union])
...
def train(args, logger):
    ...
    se = torch.load('data/' + args.dataset + '_se.pt')
    ...
    data['lap_pe'] = torch.tensor(lap_pe, dtype=torch.float32).to(args.device)
    data['pos_enc'] = torch.tensor(pe, dtype=torch.float32).to(args.device)
    data['se'] = torch.tensor(se, dtype=torch.float32).to(args.device)
    dim_pos = pe.shape[1]
    ...
    for epoch in range(args.epochs):
        ...
        for perm in DataLoader(range(train_edges.shape[0]), args.batch_size, shuffle=True):
            ...
            optim.zero_grad()
            # forward pass
            sign_flip = torch.rand(data.lap_pe.size(1)).to(device)
            sign_flip[sign_flip>=0.5] = 1.0; sign_flip[sign_flip<0.5] = -1.0
            lap_pe = data.lap_pe = data.lap_pe*sign_flip.unsqueeze(0)
            pos_score = model(adj_norm, features, pos_edges, data.com_x, data.pos_enc, lap_pe, data.se)
            neg_score = model(adj_norm, features, neg_edges, data.com_x, data.pos_enc, lap_pe, data.se)
        ...
        with torch.no_grad():
            ...
            logits_val = model.decoder(torch.cat([z[val_pairs.T[0]], data.se[val_pairs.T[0]]], dim=-1), torch.cat([z[val_pairs.T[1]], data.se[val_pairs.T[1]]], dim=-1)).detach().cpu()
            logits_test = model.decoder(torch.cat([z[test_pairs.T[0]], data.se[test_pairs.T[0]]], dim=-1), torch.cat([z[test_pairs.T[1]], data.se[test_pairs.T[1]]], dim=-1)).detach().cpu()
