...
np.random.seed(123)
device = 'cpu'
adj = pickle.load(open('data/'+data + '_adj.pkl', 'rb'))
feature = pickle.load(open('data/'+data + '_feat.pkl', 'rb')).toarray()
...
pos_enc = []
for node in range(feature.shape[0]):
    temp = []
    if node % 1000 == 0:
